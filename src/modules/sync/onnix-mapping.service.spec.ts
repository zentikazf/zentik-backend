...
describe('OnnixMappingService', () => {
  ...
  describe('resolveClientId — R15/R16', () => {
    it('R15: cliente mapeado -> devuelve el onnix_id de la tabla de mapeo', async () => {
      ...
      const id = await service.resolveClientId('zentik_client_1');
      expect(id).toBe(555);
      const arg = prisma.onnixEntityMapping.findUnique.mock.calls[0][0];
      expect(arg.where).toEqual({
        entityType_zentikId: { entityType: 'client', zentikId: 'zentik_client_1' },
      });
    });
    ...
    it('R16: cliente NO mapeado -> null (la fila ira a failed en el dispatcher)', async () => {
      ...
      const id = await service.resolveClientId('zentik_client_desconocido');
      expect(id).toBeNull();
    });
  });

  describe('resolveProjectId — R17/R18', () => {
    it('R17: proyecto mapeado -> devuelve el project_id de Onnix', async () => {
      ...
      const id = await service.resolveProjectId('zentik_project_1');
      expect(id).toBe(777);
      const arg = prisma.onnixEntityMapping.findUnique.mock.calls[0][0];
      expect(arg.where).toEqual({
        entityType_zentikId: { entityType: 'project', zentikId: 'zentik_project_1' },
      });
    });

    it('R18: proyecto NO mapeado -> null (best-effort, no bloquea, no failed)', async () => {
      prisma.onnixEntityMapping.findUnique.mockResolvedValueOnce(null);
      const id = await service.resolveProjectId('zentik_project_x');
      expect(id).toBeNull();
    });
  ...
  describe('resolveCatalogIds — R19', () => {
    it('R19: usa mapeo explicito de tipo/categoria/prioridad si existe', async () => {
      ...
      const ids = await service.resolveCatalogIds('SUPPORT_REQUEST', 'MEDIUM', TRACE);
  ...
  describe('cache de catalogos — R20', () => {
    it('R20: sirve catalogos desde cache (no llama getCatalogos por cada resolucion)', async () => {
      ...
      await service.resolveCatalogIds('SUPPORT_REQUEST', 'MEDIUM', TRACE);
